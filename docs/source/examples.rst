...
The SBOMs produced by ``debsbom`` can be further processed with ``debsbom compare``, which compares a base (reference) SBOM
with a target SBOM and outputs the components present only in the target.
...
.. code-block:: bash
   ...
   debsbom compare \
       --base-sbom sbom.old.cdx.json \
       --target-sbom sbom.cdx.json \
       --out extras.cdx.json
...
.. code-block:: bash
   ...
   debsbom compare -t spdx -b <(cat sbom.old.spdx.json) -n <(cat sbom.spdx.json) -o -
